...
public class ParseFitFeedXml {
    ...
    public ParseFitFeedXml(Integer categoryId, String fitFeedUrl, String manualDate) throws InvalidIdFault, UnexpectedErrorFault,
    ...
    public ArrayList<KeywordEntry> parseXml() throws IOException, SAXException, ParserConfigurationException, ParseException,
            ServiceException {
    ...
    private String getFitFeedXml() throws IOException, ServiceException {
        HttpGet request = new HttpGet(this.fitFeedUrl);
        HttpResponse response = request.getResponse();
        return response.getBodyAsString();
    }

    private boolean parseFitFeedXml(String xml) throws SAXException, IOException, ParserConfigurationException, ParseException {
    ...
    private void parseRss(Element rootNode) throws ParseException {
        for (Node childNode = rootNode.getFirstChild(); childNode != null; childNode = childNode.getNextSibling()) {
            if ((childNode.getNodeType() != 1) || (!childNode.getLocalName().equals("channel")))
                continue;
    ...
    private void parsePrices(Element channelNode) throws ParseException {
        for (Node childNode = channelNode.getFirstChild(); childNode != null; childNode = childNode.getNextSibling()) {
            if ((childNode.getNodeType() != 1) || (!childNode.getLocalName().equals("item")))
                continue;
    ...
    private boolean updateKeyword(String keyword, String content, KeywordEntry entry) throws SQLException, ParseException {
    ...
    private boolean insertNewKeyword(KeywordEntry entry, String keyword, String content) throws ClassNotFoundException, SQLException,
            ParseException {
    ...
    private void initFitFeed() throws InvalidIdFault, UnexpectedErrorFault, LoginFault, RemoteException, ServiceException {
        this.keywords = new ArrayList();
        this.missingMarkets = new ArrayList();
        initRegionMap();
    }

    private void initRegionMap() throws InvalidIdFault, UnexpectedErrorFault, LoginFault, RemoteException, ServiceException {
