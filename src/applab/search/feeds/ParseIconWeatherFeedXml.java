...
public class ParseIconWeatherFeedXml {
    ...
    private String getIconWeatherConditionsXml(String locationId) throws IOException {
        HttpGet request = new HttpGet(this.iconWeatherFeedUrl + locationId);
        HttpResponse response = request.getResponse();
        return response.getBodyAsString();
    }

    private String getIconWeatherForecastXml(String locationId) throws IOException {
    ...
    private void parseWeatherRequestItem(Element itemNode) throws ParseException {
        ...
        for (Node childNode = itemNode.getFirstChild(); childNode != null; childNode = childNode.getNextSibling()) {
            if (childNode.getNodeType() == 1) {
                WeatherRequestElement weatherRequestElement = WeatherRequestElement.valueOf(childNode.getLocalName());
        ...
        this.keywords.add(new WeatherConditionsKeyword("Information provided by the ICON Weather Feed Service", "Daily_and_5_day_Forecast",
                this.categoryId.intValue(), locationId, subcountyName, districtName, regionName));
    }
    ...
    private void cleanUpWeatherConditionsXml(WeatherConditionsKeyword keyword) throws IOException, SAXException,
    ...
    public void cleanUpWeatherForecastXml(WeatherConditionsKeyword keyword) throws IOException, SAXException, ParserConfigurationException,
            ParseException {
    ...
    private void parseXmlToForecasts(Element rootNode, WeatherConditionsKeyword keyword) {
        for (Node childNode = rootNode.getFirstChild(); childNode != null; childNode = childNode.getNextSibling())
        {
            if ((childNode.getNodeType() != 1) || (!childNode.getLocalName().equals("forecast")))
                continue;
    ...
    public boolean saveToDatabase() throws ClassNotFoundException, SQLException {
                ...
                if (keyword.getSubcounty() != null) {
                    ...
                    if ((keywordString != null) && (contentString != null)) {
                        if ((updateKeyword(keywordString, contentString, keyword))
                                || (insertNewKeyword(keyword, keywordString, contentString)))
                            continue;
                        success = false;
                    }
                    else {
    ...
    private boolean updateKeyword(String keyword, String content, WeatherConditionsKeyword entry) throws SQLException, ParseException {
        this.updateStatement.clearParameters();
        this.updateStatement.setString(1, content);
        this.updateStatement.setTimestamp(2, DatabaseHelpers.getTimestamp(new java.util.Date()));
        this.updateStatement.setInt(3, 0);
        this.updateStatement.setString(4, keyword);
        return this.updateStatement.executeUpdate() != 0;
    }

    private boolean insertNewKeyword(WeatherConditionsKeyword entry, String keyword, String content) throws ClassNotFoundException,
        ...
        this.insertStatement.setTimestamp(5, DatabaseHelpers.getTimestamp(new java.util.Date()));
