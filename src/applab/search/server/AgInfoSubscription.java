...
public class AgInfoSubscription {
    ...
    public String processAgInfoSubscription() throws InvalidIdFault, UnexpectedErrorFault, LoginFault, RemoteException, ServiceException {
    ...
    private HashMap<String, Subscription_Topic__c> getTopicGroupsFromDisplayOrder(String[] topicDisplayOrder) throws InvalidIdFault,
    ...
    private HashMap<String, Subscription_Topic__c> getTopicGroupsFromName(String[] topicNames) throws InvalidIdFault, UnexpectedErrorFault,
    ...
    private String getAllTopicsString() throws InvalidSObjectFault, MalformedQueryFault, InvalidFieldFault, InvalidIdFault,
            UnexpectedErrorFault, InvalidQueryLocatorFault, RemoteException, ServiceException {
        SoapBindingStub binding = SalesforceProxy.createBinding();
        StringBuilder queryText = new StringBuilder();
        queryText.append("SELECT ");
        ...
        if (query.getSize() > 0) {
            if (this.subscribe) {
                textMessage.append(this.TOPICS_INFO_SUBSCRIBE_STRING);
            }
            else {
                textMessage.append(this.TOPICS_INFO_UNSUBSCRIBE_STRING);
            }
            textMessage.append("\n");
            for (int i = 0; i < query.getSize(); i++) {
                Subscription_Topic__c subscriptionTopic = (Subscription_Topic__c)query.getRecords(i);
            ...
            textMessage.append(this.NO_TOPICS_SUBSCRIBE_STRING);
        }
        else {
            textMessage.append(this.NO_TOPICS_UNSUBSCRIBE_STRING);
        }

        return textMessage.toString();
    }

    private Person getPersonDetails(String phoneNumber) throws ServiceException, InvalidIdFault, UnexpectedErrorFault, LoginFault,
            RemoteException {
        try {
    ...
    private Person createNewPerson(String phoneNumber) throws ServiceException, InvalidIdFault, UnexpectedErrorFault, LoginFault,
            RemoteException {
        SoapBindingStub binding = SalesforceProxy.createBinding();
    ...
    private boolean addPersonGroupAssociations(Person person) throws InvalidIdFault, UnexpectedErrorFault, LoginFault, RemoteException,
            ServiceException {
        SoapBindingStub binding = SalesforceProxy.createBinding();
    ...
    private boolean removePersonGroupAssociation(Person person) throws InvalidIdFault, UnexpectedErrorFault, LoginFault, RemoteException,
            ServiceException {
        SoapBindingStub binding = SalesforceProxy.createBinding();
        ArrayList existingSubscriptions = person.getSubscriptions();
        ...
        return (this.groupsToChange.size() <= 0) || (deleteResult.length != 0);
    }
